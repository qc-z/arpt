...
var ids = new Array("selectfiles","selectfiles1","selectfiles2","selectfiles3",
    "selectfiles4","selectfiles5","selectfiles6","selectfiles7",
    "selectfiles21","selectfiles31","selectfiles41","selectfiles51",
    "selectfiles61","selectfiles71","selectfiles9527","selectfiles95271","selectfileswenmei","selectfileshengping","selectfileweizheng");
$.each(ids,function(i,n){
    ...
    //绑定各种事件，并在事件监听函数中做你想做的事.当文件添加进来时
        ...
        if(self.search('selectfiles2')===0){ // 添加广告机轮播图，不限制数量
        ...
        }else if(self.search('selectfiles9527')===0){ // 明星面对面图片
            if($('#IMG9527').find('img').length>=1){
                layer.confirm('只能上传1张图片');
                return false;
            }
            temp = "star";
            ...
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $('#IMG9527').append('<br/><img src="'+ imgsrc +'" />');
                    // $("#IMG71").css("display","none")
        ...
        }else if(self.search('selectfileswenmei1')===0){ // 纹眉神器图片
            if($('#IMGwenmei1').find('img').length>=1){
                layer.confirm('只能上传1张纹眉图片');
                return false;
            }
            temp = "tattoo";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $('#IMGwenmei').append('<br/><img src="'+ imgsrc +'" />');
                    // $("#IMG71").css("display","none")
        ...
        }else if(self.search('selectfileshengping1')===0){ // 添加横屏广告，不限制数量
            if($('#IMGhengping1').find('img').length>=2){
                layer.confirm('只能上传3张横屏图片');
                return false;
            }
            temp = "hengping";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $('#hengping1').append('<br/><img  src='+ imgsrc +' /><hr>');
                    // $("#IMG31").css("display","none")
                });
            });
        }else if(self.search('selectfileweizheng1')===0){ // 添加微整，2张
            if($('#IMGwenmei1').find('img').length>=2){
                layer.confirm('只能上传2张微整图片');
                return false;
            }
            temp = "weizheng";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $('#weizheng').append('<br/><img  src='+ imgsrc +' /><hr>');
                    // $("#IMG31").css("display","none")
                });
            });
        }
        ...
        }else if(self.search('selectfiles95271')===0){ // 实时预览图片
            temp = "star";

            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $("#IMG95271 img").css("display","none")
                    $("#IMG95271 br").css("display","none")
                    $('#IMG95271').append('<br/><img src="'+ imgsrc +'" />');
                    // $("#IMG71").css("display","none")
        ...
        }else if(self.search('selectfileswenmei')===0){ // 纹眉神器图片


            temp = "tattoo";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $("#IMGwenmei .imgnone").css("display","none")
                    $("#IMGwenmei .hrnone").css("display","none")
                    $('#IMGwenmei').append('<br/><img src="'+ imgsrc +'" />');
                    // $("#IMG71").css("display","none")
        ...
        }else if(self.search('selectfileshengping')===0){ // 添加横屏广告，3
            
            temp = "hengping";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $("#hengping .imgnone").css("display","none")
                    $("#hengping .hrnone").css("display","none")
                    $('#hengping').append('<br/><img  src='+ imgsrc +' /><hr>');
                    // $("#IMG31").css("display","none")
                });
            });
        }else if(self.search('selectfileweizheng')===0){ // 添加微整，2张
            
            temp = "weizheng";
            plupload.each(files, function(file) {
                previewImage(file,function(imgsrc){
                    $("#weizheng .imgnone").css("display","none")
                    $("#weizheng .hrnone").css("display","none")
                    $('#weizheng').append('<br/><img  src='+ imgsrc +' /><hr>');
                    // $("#IMG31").css("display","none")
                });
            });
    ...
    uploader.bind('FileUploaded', function(up, file, info) {
        ...
        if (info.status == 200)
        {
            ...
            if(temp == "skin"){
                ...
                imgObj.tattoo = imgUrl

            }else if(temp == "hengping"){
                imgObj.hengping.push(imgUrl)

            }else if(temp == "weizheng"){
                imgObj.weizheng.push(imgUrl)
...
$('#editPostfiles').click(function(){
    ...
    //     if($('#IMG4').find('img').length){
        ...
        var data = {
            ...
            "info": imgObj.info,
            "roll": imgObj.roll,
            "tattoo": imgObj.tattoo,
            "hengping": imgObj.hengping,
            "weizheng": imgObj.weizheng
    ...
    $.ajax({
            type: "GET",
                ...
                success: function (res) {
                    ...
                    localStorage.setItem("live",res.results.star);
                    }
...
function editAr(msg){
    ...
    $("#IMG95271").append('<img style="width:25rem" src='+msg.star+'>');
    var imagestr = "";
    ...
    $("#IMGwenmei").append('<img  class="imgnone" style="width:25rem" src='+msg.tattoo+'>');
    var imagehengping = "";
    if(msg.hengping && msg.hengping.length){
        for(var i = 0;i<msg.hengping.length;i++){
        imagehengping += '<img style="width:25rem"; class="imgnone" src='+msg.hengping[i]+'><hr class="hrnone">'
    }
        $("#hengping").append(imagehengping);
    }
    var imageweizheng = "";
    if(msg.weizheng && msg.weizheng.length){
        for(var i = 0;i<msg.weizheng.length;i++){
        imageweizheng += '<img style="width:25rem"; class="imgnone" src='+msg.weizheng[i]+'><hr class="hrnone">'
    }
        $("#weizheng").append(imageweizheng);
    }
...
window.onload = function(){
    ...
    $(".dropdown-menu").on("click","a",function(){
        ...
        if($(this).attr("data-type") == "skin"){
            ...
            success: function (res) {
                console.log(res)
            $('.arTop img').attr("src",localStorage.getItem("star"))
