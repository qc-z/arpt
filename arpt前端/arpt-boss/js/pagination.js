...
function getUsers(/*p*/element) {
    ...
    if(/*p*/element=='yesterday'){
        reqdata.code=1;
    }
    if(/*p*/element=='mobile'){
        reqdata.code=2;
        reqdata.mobile=$("#inputmobile").val();
    }
    req('getUsers',reqdata,function (result) {
        var temp=$("#user");
        $("#users").html("");
        for(var i in result)
        {
            if(i<pagenumber*i && i>pagenumber*(i+1)){
                var t=temp.clone();
                t.children("#photo").css('background-image','url("'+result[i].imgurl+'")');
                t.children("#mobile").html(result[i].mobile);
                t.find("#view").attr('href',"customer-message.html?uuid="+result[i].uuid);
                $("#users").append(t)
            }
